...
impl WindowsDimmer {
    ...
    /// Post the shutdown message and wait for the worker, bounded.
    ///
    ...
    ///
    /// So the wait is on the `finished` channel, which disconnects when the
    /// thread returns, and `teardown::teardown_decision` turns the outcome into
    /// join-or-detach.
    ///
    /// **Detaching is a real cost and is not hidden**: the thread keeps running,
    /// its overlays stay up until whatever destroys them does, and any OS
    /// resources it holds are released then rather than by us. On a quit - the
    /// caller this is usually - that is the process image going, moments later.
    /// It is **not only** the quit path, though, and the difference matters:
    /// `duja-app` also drops the dimmer mid-session when a backend retires, and
    /// there a detached worker's overlays would outlive the retirement with the
    /// app still running. That arm is unreachable on Windows today - every
    /// producer of `DimmerError::Unsupported` is under `linux/` - so it is a
    /// caveat rather than a second bug, and an earlier draft of this paragraph
    /// said "until the process image goes" flatly, which is wrong for exactly
    /// that case.
    ///
    /// Either way it beats not returning, which is what the alternative was.
            ...
            match teardown_decision(self.finished.recv_timeout(SHUTDOWN_BUDGET)) {
                ...
                Teardown::Detach => {
                    warn!(
                        ...
                        "overlay worker did not stop in time; detached, its overlays remain"
                    );
                    drop(join);
                }
            }
