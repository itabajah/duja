...
| [D-045](#d-045) | ~~v0.1.1 (`#49`)~~ | `duja-dimmer` `win/mod.rs` | ~~`shutdown()`'s `join()` is unbounded if the overlay worker is wedged in a hung Win32 call~~ - **drained in `#160`** with the detach-or-timeout shape the row asked for, and the decision pinned on all three lanes |
...
| [D-113](#d-113) | ~~P8 gate~~ | `duja-core` `config/persist.rs` | ~~The config size cap is asymmetric: the read side refuses what the write side will happily produce, and the state path then overwrites what it could not read~~ - **drained in `#158`**, in all three halves the row named, including the UI one it said a write-side cap alone would not close |
...
~~**Both xtask subcommands take `std::env::Args`, and a test cannot choose what is in one.**~~ - **drained in `#156`**, the first change after the tag, exactly where the row said it belonged. `size::run` and `dist::run` now take `impl Iterator<Item = String>`, which `std::env::args()` already satisfies, so `main()` itself needed no change; `size`'s argument loop moved into an `Invocation::parse` on `dist`'s model, the shared rule moved to a new `xtask/src/args.rs`, and `size.rs` went from **60.10 %** of regions to **79.38 %** while the crate's suite went from **48 tests to 61**.

*(**The per-file counts that used to be in that sentence are deleted rather than corrected again**, which is this row's own rule and it took the checkpoint PR four review rounds to reach. The sentence said `size.rs` gained seven tests, `args.rs` four, the suite 48 to 59, and coverage 77.36 %, and all four were correct when written. `#156`'s **own** third review round then added two tests - one in `size.rs`, one in `main.rs` - and nobody re-ran the figures, so three of the four went stale: seven became eight, 59 became 61, 77.36 % became 79.38 %. `args.rs` four was right then and is right now, which is why "every one staled" was wrong when this note first said it.*

*Fixing them one at a time took three passes and never converged: 59 first, then 77.36 %, then `seven` plus a `main.rs` parenthetical claiming "nothing else" - by which point `48 + 7 + 4` no longer equalled the 61 further along the same sentence. The per-file test counts **in that sentence** are what is gone; `48 to 61` is the crate's suite, measured at the merge, and `size.rs`'s coverage figures stay because they are the row's subject.)*
...
**Getting that paragraph right took three review rounds, and what they found is the record worth keeping.** It first said *twelve* tests drove the parser - which is `grep -c '#\[test\]'` on `dist.rs`: a real measurement of the **wrong quantity**, used as though it answered this question. Different from a right number going stale, which is what happened to the figures above, and this row now carries both failures. That is precisely the error this entry convicts the original row of, committed inside the correction that makes the accusation. It then said `dist`'s parsing was "already tested", which overstates in the other direction; the paragraph below has what is actually true. No draft-by-draft accounting is given **for this paragraph**, because the substance is not improved by knowing which round produced which sentence. The numbers above are the exception and are accounted for round by round, because there the sequence *is* the lesson: the rounds spent correcting those counts one at a time are what the rule against correcting a count again is for.
...
**Drained in `#158`**, in all three of the halves the row named.

**The write side.** `write_atomic` now refuses anything over `MAX_CONFIG_LEN` **before** it creates a directory or writes a byte, so Duja can no longer produce a file it will subsequently refuse to read. The placement is deliberate and pinned by its own test: after `create_dir_all`, a first run that failed would leave an empty config directory behind, which reads to the next launch and to a user looking at their disk as though Duja had got further than it did. A second test pins that *exactly* the cap round-trips, because two caps written as two comparisons is two places for an off-by-one and the failure it produces is this row's headline.

**The state path, and the remedy this row asked for turned out to be the wrong one.** The row said the state path "needs to refuse to overwrite what it could not read". The first version did exactly that, and a review priced the two failures it covers: `ConfigError::TooLarge` needs somewhere between **thirteen and fifteen thousand** entries - measured through `StateFile::save`, where a ten-character id costs 68 bytes and a twenty-one-character one 79, so the count depends on the ids; an earlier draft gave a single "~71 bytes" that no id length in the tree produces. Essentially unreachable either way, and the case the refusal was argued from. A `Deserialize` failure needs **one bad byte** in a hand-edit, which is entirely reachable and where the *old* behaviour self-healed. Refusing there means levels silently never persisting again, every session, over a typo, with no UI signal at all: the settings banner reports `config.toml` writes and this is a different file. So those two take the remedy the first version listed as "considered and not taken" - rename to `state.toml.unreadable` and start fresh, which preserves the file **and** keeps persistence working.

**But only those two, and getting that wrong was the next mistake.** The version that introduced the quarantine applied it to *every* error `StateFile::load` can produce, including `Io` - so antivirus holding the handle for a second, a network share hiccup or a sharing violation would have moved the user's file aside for a condition that clears on its own. A new destructive behaviour, introduced in the name of removing one. The distinction is whether re-reading gives the same answer: for a size or a parse it will, and for invalid UTF-8 - which the read path reports as `Io(InvalidData)` - it also will, so that kind is promoted and every other `Io` refuses to write and touches nothing. Narrower than the criterion it serves, knowingly: a `PermissionDenied` would also fail the same way every launch, but "cannot read it" and "cannot move it" are usually the same permission, and a wrong promotion costs the file where a wrong refusal costs a session. It is the one case where the safe answer is still the unhappy one, and it is a residual rather than a fix. If a quarantine name cannot be found or the rename itself fails - a read-only directory, a permission problem - the same refusal applies, which is the narrow case the original remedy was always right for. **An occupied *file* is not one of those**, and two drafts got the exception wrong in turn: `fs::rename` replaces an existing destination file on every supported platform, so a second corruption would have silently destroyed the first quarantined copy - while a destination *directory* blocks the rename whether it is empty or not (measured on Windows; POSIX makes `rename(file, dir)` `EISDIR` regardless). Taken names now get a numeric suffix, bounded, and running out of them is what refuses.

**And the criterion the quarantine turns on was wrong twice before it was right.** It first matched every error, including `Io`, so a transient failure - antivirus holding the handle, a network share hiccup - would have moved the user's file for a condition that clears on its own. Narrowing it to the `TooLarge` and `Deserialize` **variants** then missed that `read_capped` maps **invalid UTF-8 to `Io(InvalidData)`**, so a `state.toml` saved as UTF-16 took the transient arm and levels silently never persisted again, every session: verbatim the outcome this design exists to remove, reached by a different door. The test is on the io **kind** now, and `InvalidData` is the file being wrong while everything else is the world being busy.

*(The same round found more in the quarantine itself. `Path::with_extension("unreadable")` turns `state.toml` into `state.unreadable`, not `state.toml.unreadable`: the code and three sentences of prose disagreed, and the tests sided with the code because they built the expected name with the same call, so nothing could catch it. And the test for the transient arm asserted only that a write was refused, against a *directory* - which is unwritable anyway - so flipping that arm's `blocked` reddened nothing in the workspace. It asserts the flag directly now, from the child module where it was already in scope. The rename-failure test used a directory at the path - which produces an `Io` error, so once the `Io` arm existed it passed through the wrong branch entirely and proved nothing.)*

*(An aside worth keeping: this row's "read as garbage" is not what happens. An over-cap `state.toml` is not read at all - the cap refuses it and defaults are substituted - and two drafts of the fix repeated the row's phrasing without checking it.)*

**The UI half, which the row insisted on and was right to.** "A write-side cap alone would turn 'settings writes fail' into 'settings writes fail slightly earlier'." It was worse than that: both settings-write sites ended `Err(e) => warn!(...)`, so a failed write **looked exactly like a successful one** - the view-model had already reflected the toggle, so the switch stayed where the user put it, and the setting was simply back where it started at the next launch. The settings window now carries a banner naming the file and the error.

**Only `Ok(true)` clears that banner, and the first version cleared on `Ok(_)`.** `persist_config_change` answers `Ok(false)` for a command with no config footprint at all - `SetInput`, `CheckUpdates`, a `ClearHotkey` with nothing bound - so under the first version a user whose write had just failed could pick a different **input source** one row below in the same window and watch the warning vanish while their setting stayed unsaved. That is the same lie this half exists to stop, pointing a third way, and it was the easiest of the three to ship.

**Two residuals worth naming.** `blocked` stops the **update-check timestamp** persisting as well as the levels, because both live in `state.toml` - so a session that refuses to write also re-checks for updates at every launch. Both log lines say so now. And the first version's second deferral reason for the quarantine, "it needs somewhere to say so in the UI", is still unanswered: a moved file's only signal is a `tracing::warn!`.

**What is *not* covered, stated rather than implied by "all three halves".** `set_dimming_enabled` is driven from the **flyout** toggle, and its banner lands in the settings window, which is usually closed when the flyout is open. A flyout dimming toggle that fails to persist is still silent at the moment it happens. The flyout is deliberately a minimal brightness popup, so giving it an error surface is a design decision rather than a fix - named here instead of guessed at
...
**Drained in `#159`**, by the seam this row's own re-triage predicted and the fixture that landed with [D-040](#d-040) did not supply.

`a_platform_wake_invalidates_the_ramp_and_re_applies` drives `on_platform_wake` against a recording gamma channel and asserts **both** of its load-bearing lines: that the ramp was declared stale, and that something then actually re-applied. Deleting either one reds it, which was the whole gap - `invalidate` alone changes nothing until some later event triggers a batch, and a resume that changes no display produces no snapshot, which is exactly the case that was broken.

**Why the `AppState` fixture was not enough, which is the part worth keeping.** `#157` made `AppState` constructible and this row stayed open. Its gamma channel was the *real* one made inert by a resolver that answered `None` for every id: safe - the sink returns `false` before any OS call, writes no marker, touches no ramp - and **blind**, because a batch that would have been rewritten and one that would have been diffed are indistinguishable from outside a sink that refuses everything. Safety was never the hard part. `#159` gives `GammaBackend` the same enum-and-fake shape `PlatformTray` got, and the recording channel is what makes the phases observable.

**The `AppState::dimmer` change this row named turned out to be needed too, and mechanically so.** The field was a concrete `Option<PlatformDimmer>`; every use of it already went through the `Dimmer` trait and `apply_overlays` coerced to `&mut dyn Dimmer` on the next line, so `Option<Box<dyn Dimmer>>` costs one allocation at startup and buys a fixture that can hold `duja_core::testing::FakeDimmer`. It needed one non-obvious line at the call site: `Box<dyn Dimmer>` is `Box<dyn Dimmer + 'static>`, so a bare `as_deref_mut()` hands `apply_batch` a borrow that outlives the call and then blocks the `self.dimmer = None` retirement below it. An explicit reborrow in a scope that ends first is what keeps both, and it is commented there rather than left to be rediscovered
...
**Drained in `#159`**, on the recording gamma channel `#157`'s fixture lacked.

Two tests, and the split matters. `the_batch_carries_the_planned_command` asserts the batch carries the planned command for the display the user drove - not merely that a call happened, because an empty batch is what a call site that stopped planning also produces. `the_overlay_backend_is_carried_by_the_batch_rather_than_driven_beside_it` asserts the overlay backend reached `apply_batch`, and that is the one that catches the historical spelling: a call site doing `dimmer.apply(&commands)` itself and then asking the channel for its gamma phases still records a batch, but with **nothing left to hand through**. The flag is what separates "routed through" from "called alongside".

Proven red by re-inserting exactly that shape at `apply_overlays`. Only the second test reds, which is the correct answer rather than a lucky one: the first is about *what* is planned and the defect does not change that.

**The mitigation this row recorded does not hold, and a review falsified it with this drain's own reproduction.** The row said `GammaBackend` exposes no gamma-only `apply`, so "the wrong order cannot be written without also re-adding a method". It can: `dimmer.apply(&commands)` followed by `gamma.apply_batch(&commands, None)` uses only what is already there, drives the overlay to completion *before* the engage phase, and restores the flash. The API shape prevented one **spelling** of the wrong order, not the wrong order. So the tests are not a pin on top of a working mitigation - they are what protects the property, and the shape is a convenience. *(A first correction fixed this in the test's doc and left the paragraph here saying the opposite, in a commit whose message claimed both. Two claims about one hazard, in one PR, is worse than the original.)*

**And the reason this did not drain with [D-040](#d-040)** is worth stating once more, because the row above shares it: an `AppState` you can build is not the same as a gamma channel you can see. `#157`'s fixture had the first and made the second impossible by construction, since its sink refused every id
...
**Drained in `#160`**, with the shape the row named: detach-or-timeout, because stable `std` still has no timed join.

The mechanism is a channel the worker holds and never sends on. Its sender drops when the thread returns - normally *or* by unwinding - so `recv_timeout` on the other end distinguishes "finished" from "still going" without the worker cooperating, which is the point: a worker that could cooperate would not be wedged. The **decision** taken on that answer lives in a new cross-platform `duja_dimmer::teardown`, so it is tested on all three lanes rather than only where the wedge can happen (ADR-0011). *("Compiles and is tested on all three" is what the first version said, and it did not: its only non-test caller is `cfg(windows)`, so a plain `--lib` build on the other two lanes has nothing but dead code and `clippy -D warnings` fails. `mac_geom` - the module this one was modelled on - carries a `cfg_attr(not(target_os = "macos"), allow(dead_code))` for exactly that, and the copy omitted it. The claim that broke the lanes was the claim about lanes.)* Making it answer `Join` for every outcome reds three of its four tests.

**And the honest limit of that proof, which a review had to establish.** `Teardown::Join` for every outcome is *not* the historical behaviour; the unbounded `join()` in `shutdown` is. Re-inserting **that**, at the site it historically occupied, leaves the whole suite green - and so does breaking the mechanism by dropping the sender at the top of the closure instead of holding it. What is pinned is a pure function this fix introduced, which never carried the bug. That is `#82`'s shape, and the first version of this paragraph called the pure mutation "the historical behaviour", which is the label that makes it look otherwise.

Closing it needs a `windows_live` test that holds the worker busy and watchdogs the call, on the model of `duja-app`'s `shutdown_does_not_hang_on_a_blocked_worker` - which does exactly that for the engine's equivalent, one crate over, so "untestable" is not available as an excuse and is not claimed. It is a live test against a real message loop, which is why it belongs with the other `windows_live` work rather than in the change that made the wait bounded.

**Detaching costs something and the doc says so rather than implying a clean fix.** The thread keeps running, its overlays stay up, and any OS resources it holds are released by process exit rather than by us. On the quit path that is moments - but it is not only the quit path, since `duja-app` also drops the dimmer mid-session when a backend retires, and there a detached worker's overlays would outlive the retirement. That arm is unreachable on Windows today (nothing under `win/` produces `DimmerError::Unsupported`), so it is a caveat rather than a second bug. Both docs used to say the overlays last "until the process image goes", which is wrong for exactly that case; they say "until whatever destroys them does" now, and `shutdown`'s names the retirement path explicitly. *(A first correction wrote that sentence as though the edit had been made, and it had not - the claim about the docs was in the archive and the docs still said the old thing.)*
