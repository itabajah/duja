...
The mechanism is a channel the worker holds and never sends on. Its sender drops when the thread returns - normally *or* by unwinding - so `recv_timeout` on the other end distinguishes "finished" from "still going" without the worker cooperating, which is the point: a worker that could cooperate would not be wedged. The **decision** taken on that answer lives in a new cross-platform `duja_dimmer::teardown`, so it compiles and is tested on all three lanes rather than only where the wedge can happen (ADR-0011). Making it answer `Join` for every outcome reds three of its four tests.
