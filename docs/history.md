...
Moved here from [plan.md](plan.md) when the phase closed, exactly as P7's table
was when P8 opened and P8's was at the `v0.1.6` checkpoint. That file's opening
rule is that anything already done is described here rather than there, and this
section was 260 of its 399 lines.
...
<a id="s66"></a>
### Wave 5 - the two rows left over, and what a test cannot do

The phase ended on the two rows its own waves had not touched:
[D-059](debt-archive.md#d-059), which wave 1 was expected to drain and did not,
and [D-076](debt.md#d-076) from wave 4. **Neither closed on the mechanism its own
row proposed**, and the two failures are different enough to be worth separating.

**[D-059](debt-archive.md#d-059) drained on a type, and no test could have done
it.** The row asked for "an injected 'is the loop running' probe, or a
`TrayBuilder` trait". Both are tests. What the row was protecting against is
`build_tray`/`init_hotkeys` being moved back into `run`'s pre-loop phase - and a
test observes what a function *does*, not that code stayed where it was. Any
probe would have pinned the seam it was injected into and said nothing about a
caller that stopped using it.

The row's claim was measured before it was fixed rather than repeated: with the
defect restored at the site it occupied before the event-loop-first restructure,
**377 `duja-app` tests pass**, `tests/loop_time_assembly.rs` among them, because
that file pins the mechanism and never the wiring to it. `#167` replaced the
remedy with a witness type. `build_tray` and `init_hotkeys` take a
`&LoopRunning`; its field and constructor are private to the module that owns the
`Timer::single_shot`; the only mint is inside the queued callback. Re-inlining is
`error[E0061]` at both sites, and forging one from `tray.rs` is `error[E0603]` -
with rustc naming the single edit that would reopen the hole. What the type
cannot say is that the callback runs *in-loop*, since that rests on two pinned
dependency versions dependabot bumps unattended, so `tests/loop_running_token.rs`
pins it against the real stack, with both assertions proven falsifiable by
separate mutations.

**[D-076](debt.md#d-076) narrowed rather than drained, and the honest half is the
half that did not close.** The unbounded wait its title names is gone: a
non-blocking `connect` makes Linux answer `EAGAIN`, and that errno is *positive
evidence of a listener* rather than a timeout to wait out, so the wait was removed
rather than shortened. The BSD half cannot be fixed by any wait - `ECONNREFUSED`
there means either a dead inode or a full backlog, and a budget long enough to
tell them apart is longer than the bind lock the probe is held under. A retry loop
was considered and **deliberately not built**, because it would have failed in the
realistic case while reading as a fix.

Both of its hazards were proven red **on CI**, since `unix_socket.rs` is
unreachable on the Windows dev box and there is no WSL distro on it. Run
[31308699587](https://github.com/itabajah/duja/actions/runs/31308699587), ubuntu
...
|---|---|---|
| 1 | the guard is dead code on macOS; the retry loop is unreachable *and* wrong; an overclaimed reachability argument | the original |
| 2 | the header fix asserted safety for the open half; a partial kernel read stated as exhaustive; a false compile-time guarantee | round 1's fix |
| 3 | "both arms dead" - both kernels resolve with `FOLLOW`, `symlink_metadata` does not | round 2's fix |
| 4 | an assertion that cannot fire; the route not pinned; a premise deleted but its conclusion kept | round 3's fix |
| 5 | the section heading kept the claim four rounds had removed | round 4's fix |
| 6 | a citation quoting a `git log` taken on the wrong branch | round 5's fix |

**The root error is worth naming, because it was not a slip of wording.** The
original read FreeBSD's `unp_connectat` where XNU's `unp_connect` was the relevant
source. XNU rejects a non-socket with `ENOTSOCK` rather than `ECONNREFUSED`, so
the ownership guard was never reached on macOS - dead code, whose test passed on
that lane and would have passed identically with the guard deleted, while three
places said it closed on both platforms. One wrong kernel produced a false claim,
a vacuous test on one lane, and a debt row that overstated its own remedy.

Round 3 is the one that paid for itself twice. It established that the
socket-type arm is **live** on macOS after all, because both kernels resolve a
connect path with `FOLLOW` while `symlink_metadata` does not - which turned a
claim that needed two kernel sources to check into something a test could make.
That test now runs on both lanes and fails on both if the guard is deleted. Round
4 then found the test contained an assertion that could not fire: `remove_file` is
`unlink(2)` and never follows the final symlink, so under the very mutation the
test guards against, that assertion passed while the other two failed. **A test
written to settle an argument about a check that cannot fail contained a check
that cannot fail.**
...
- **The commit-lint job rejects merge commits outright**, and `--no-merge-commit`
  ...
  squash-merge only, so a merge commit on a feature branch is a policy violation
  rather than a formatting nit - `STATUS.md` calls the job "advisory", and it is
  advisory only in the sense that the PR title is what lands.
...
question unanswered: nothing has run `build_tray` on a CI runner's window station.

That question is much less pressing than it was - after `#157` no test builds a
...
- **[D-102](debt.md#d-102)'s cheap experiment.** One `#[ignore]`d test that
  ...
  ([D-016](debt-archive.md#d-016), [D-040](debt-archive.md#d-040), [D-059](debt-archive.md#d-059),
...
[D-059](debt-archive.md#d-059), [D-065](debt-archive.md#d-065)) all defer on "`AppState` cannot
